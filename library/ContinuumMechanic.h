...
template <typename T, Index n> struct MediumCon {
  ...
  // Lagrange's description of velocity:
  StaticVector<T, n> velocityL{};
  // Euler's description of velocity:
  StaticVector<T, n> velocityE{};

  MapMatrix deformMapDual{};
...
public:
  ...
  MediumCon(const MapMatrix &motionEquation,
      ...
      : ref(refConfig), deformMapDual(motionEquation), hasRef(true) {}
  ...
  MediumCon(const MapMatrix &inverseMotionEquation,
    ...
    deformMapDual = [inverseMotionEquation](Dual t) -> Matrix<Dual, n, n> {
  ...
  Matrix<T, n, n> deformMap(T t) const {
    if (!deformMapDual)
      throw std::logic_error("deformMapDual is not set.");
    return valuePart(deformMapDual(Dual{static_cast<double>(t), 0.0}));
  }
  ...
  Matrix<T, n, n> dKdt(T t) const {
    if (!deformMapDual)
      throw std::logic_error("deformMapDual is not set.");
    return derivativePart(deformMapDual(Dual{static_cast<double>(t), 1.0}));
  }
  ...
  Matrix<T, n, n> d2Kdt2(T t) const {
    ...
    return secondDerivativePart(
        deformMapDual(Dual{static_cast<double>(t), 1.0, 0.0}));
  }
