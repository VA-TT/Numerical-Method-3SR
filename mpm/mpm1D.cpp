...
constexpr double eleLength(L / nElements);
...
Vector<double> N_p(nNodes), dN_p(nNodes);
...
int main() {
  ...
  for (Index i{0}; i <= nPoints; ++i) {
    ...
    double N1_x_val = N_x[0][0](x);
    double N2_x_val = N_x[1][0](x);
  ...
  for (Index step{0}; step < nSteps; step++) {
    ...
    double x_p_analytical = position_solution(0.5, time);
    double v_p_analytical = velocity_solution(0.5, time);
    positions.push_back(x_p_analytical);
    velocities.push_back(v_p_analytical);

    // Recalculate shape functions at material point's current position (from
    // MPM) (TODO: should check which element contains x_p)
    ...
    f_ext_i.resetZero();
    // f_i,int = V_p*B_i(x_p)*sigma_p (body force + nodal force)
    f_int_i.resetZero();
    for (Index p{0}; p < nMPs; p++) {
      for (Index i{0}; i < nNodes; i++) {
        f_int_i[i] += -dN_p[i] * V_p * sigma_p; // m_i = Σ_p N_i(x_p) * mass_p
      }
    }
    ...
    for (Index i{0}; i < nNodes; i++) {
      if (m_i[i] > 1e-12) { // Avoid division by zero
    ...
    for (Index i{0}; i < nNodes; i++) {
      strain_rate_p += dN_p[i] * v_i[i]; // Mapping strain rate from the node
    }
    dStrain_p = strain_rate_p * dt;
    sigma_p += E * dStrain_p; // Elastic
  }
