...
Thanks to first-order Taylor expansion again, we obtain the development of sum of the force, which should be approching 0, at iteraion \texttt{k+1} (or in another word, at when node \textbf{C} would take an increment in position into $\vec{x}+ \delta \vec{x}$):
 \begin{equation}
  ...
  \label{TaylorK+1}
   ...
   \begin{equation}
   \nabla\vec{\mathcal{F}}(\vec{x}) = -\dfrac{d\mathcal{A}^i}{dl^i}\left( \vec{e}^i(\vec{x}) \otimes \vec{e}^i(\vec{x}) \right) - \dfrac{\mathcal{A}^1\left(l^i(\vec{x})\right)}{l^i(\vec{x})} \left( \mathbb{I} - \vec{e}^i(\vec{x}) \otimes \vec{e}^i(\vec{x})\right)
  \label{Jacobian}
   \end{equation} 
...
\left(\nabla\vec{\mathcal{F}}(\vec{x}^k)\right)@\delta \vec{x}^k =  - \vec{\mathcal{F}}(\vec{x}^k)
  \label{finalConsole}
   \end{equation}
   The first iteration starts from the initial condition of the system, then the iteration begins until $\vec{\mathcal{F}}(\vec{x}^k) < \epsilon$ to be considered as 0 like discussed above.
