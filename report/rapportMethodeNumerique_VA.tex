...
The problem of the small deformation truss can be stated:
...
\begin{equation}
  ...
  \label{weakformFEM1D}
\end{equation}
where $v \in V0$ is the virtual field that we can choose such that $v(0) = 0$ to get rid of the reaction part in \cref{weakformFEM1D}. 
Or rewrite in another linear compacted form:
\begin{align}
  \forall v \in V, a(u,v) + b(R,v) &= L(v) \\
  \text{with: }
  a(u, v) &= EA \dfrac{du}{dx}\dfrac{dv}{dx}dx;\\
  b(R,v) &= -Rv(0) = 0;\\
  L(v) &=  \int^l_0fvdx + Fv(l)
  \label{weakformFEM1Dcompacted}
\end{align}
The displacemen field $u(x)$, similarly $v(x)$ is approximated by discretizing via Galerkin's method:
    \begin{equation}
u(x) \approx u^a(x) = U^i N^i  \label{Galerkin}
   \end{equation}
...
\section{Validation}
\subsection{Console}
The initial position $x_0 $is, inituitively, taken by the intitial configuration to assure better convergence and avoid unnecessary numbers of iteration. 
...
At the beginning of iteration number 5, noticing that the sum of forces acting on node C and the its displacement is already approched to a value less than $\epsilon = 10^{-10}$ , the process stoped and the final value was taken at the last step, which is iteration number $4^{text}$.
...
Though this phenomenon is expected, as in the annexxe A of \citep{dalpont2020_nonlinear_mech}.
...
This leads to the application of distributed load and concentration load exhibited on the bar, manipulated by the two function \texttt{rhsFunction() \& applyBC()}.
