...
template <typename T> struct ElementL2 {
  // Variables
  Node1D<T> x1{}, x2{}; // real coordinates
  Index n1{}, n2{};     // nodes' ID in Mesh, will be asigned

  // Jacobian: J = dx/d(xi) = (x2-x1)/2 = const
  T jacobian{}, midPoint{};

  // Mapping deriviation: dN/dx = dN/d(xi) * d(xi)/dx = dN/d(xi) * 1/J = const
  T dN1_dx{}, dN2_dx{};

  // Ctor
  ElementL2(T p1, T p2)
      : x1{p1}, x2{p2}, jacobian{(x2 - x1) / T{2}}, midPoint{(x2 + x1) / T{2}},
        dN1_dx{-T{0.5} / jacobian}, dN2_dx{T{0.5} / jacobian} {
    assert(x2 > x1 && "Element coordinates must satisfy x2 > x1");
  }

  T N1_ref(T xi) const { return (1 - xi) / 2; }
  T N2_ref(T xi) const { return (1 + xi) / 2; }
  ...
  T integrationGauss1D_ref(Func f_xi, int n = 2) const {
    ...
    for (int i = 0; i < n; ++i) {
      I +=
          jacobian * static_cast<T>(w_ptr[i]) * f_xi(static_cast<T>(xi_ptr[i]));
    }
    return I;
  }
};
...
template <typename T> struct ElementQ4 {
  ...
  ElementQ4(StaticVector<T, 4> xCoords, StaticVector<T, 4> yCoords)
      : x_nodes{xCoords}, y_nodes{yCoords} {}
