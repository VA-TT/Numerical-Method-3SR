...
int main() {
  ...
  //   std::cout << det(assemblyStiffnessK);
  std::cout << "Flag3" << std::endl;
  Vector<double> increment_displacement{
      solveLinearSystem(assemblyStiffnessK, forceF)};

  std::cout << "displacement vector U: " << increment_displacement << std::endl;
  //   // Update the position
  // Total displacement (assign the computed increment as the total for now)
  Vector<double> totalDispalcementU = increment_displacement;
  ...
  // allocate reaction per node (nNodes x d) and compute reactions for the
  // imposed nodes
  ...
  for (Index idx = 0; idx < nodeImposed.size(); ++idx) {
    ...
    for (Index k = 0; k < d; ++k) // Encastre tous d directions
    {
      ...
      // nodeImposed
      reactionR[n][k] = penalty * (totalDispalcementU[diagonalIndex] -
